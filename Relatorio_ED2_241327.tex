...
and DRL 34.1\% (26.7--42.3), a statistically null difference (McNemar
$p=0.320$): the single-run DRL advantage does not replicate. A policy that cannot
...
battery, and under the same model executing a single task costs as much as
about 720 LLM decisions. The operative cost of semantic compliance is therefore
...
LLM 13.1\%) sits near the $\approx$16\% Little's-Law prediction
...
here is set by task arrivals: at $\lambda=4$~tasks/min the mean inter-arrival
time is 15~s, of which the LLM's median decision uses 6.4\%, so all four engines
...
with a one-hot encoding of those rules (16-dimensional state). The unseen set
...
BASELINE 14/0, $p=0.0001$). DRL-OH and SLM both reach 50\%, but decomposing by
obligation shows a discard reflex rather than rule reading: both discard all
five fault tasks, yet on the fifteen that require routing to a specific region
they solve 5 each, against the heuristic's 6 and the LLM's 15. The distinction
...
the rule table reverted to the heuristic and both DRL-OH and the SLM fell back
on a discard reflex.
...
microseconds; the cloud LLM needs about a second, well within the 15~s arrival
...
($\lambda=4$~tasks/min, 12--14\% RAM occupancy), so the resource constraint never
...
the SLM from DRL-OH. Per-decision energy is a declared model, not a
...
windows ($\geq$2~h) spanning several eclipse cycles, and loads
($\lambda\geq12$~tasks/min) high enough for RAM to bind.
