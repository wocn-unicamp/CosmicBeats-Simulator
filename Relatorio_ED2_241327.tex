...
LLM keeps 20/20 while the rule table collapses to the heuristic and the SLM,
limited by its output-token budget, to 50\%. Decision energy, under a declared model, is orders of magnitude
...
\begin{enumerate}
  ...
  \item An information-parity evaluation: a rule table and a DRL agent given
    ...
    SLM is limited by its output-token budget. All contrasts are reported with paired
...
act on. The SLM reaches 50\%: it discards all five fault tasks, yet on the
fifteen that require routing to a specific region it solves 5, against the
heuristic's 6 and the LLM's 15. Instrumented re-runs trace a third to a half of
its region errors to answers truncated by its 1,024-token output budget, so this
figure reflects the configuration as much as the model. The distinction
that matters is therefore which language model and configuration, not language
model versus numerical optimiser.
...
SLM was limited by its output-token budget.
