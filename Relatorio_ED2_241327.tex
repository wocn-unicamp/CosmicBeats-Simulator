...
\begin{enumerate}
  ...
  \item A DRL agent trained with SB3 DQN over 50{,}000 synthetic steps using
    ...
    (34.1\%, 95\%~CI 26.7--42.3). The limit is therefore the observation space,
...
20.1--34.8) and DRL 34.1\% (26.7--42.3), with the paired difference
statistically null (McNemar $p=0.320$). The single-run DRL advantage over
...
A policy that cannot read the rule can do no better, in expectation, than the
probability that a single fixed action is the right one. With the three rule
types equiprobable, that bound is $1/3$, whichever action the policy settles
on---and it is reached, not exceeded, by always discarding. The pooled DRL
figure, 34.1\% (26.7--42.3), sits on that bound; in the single run it was
37.5\% (3 of 8), whose apparent margin the wide interval at $n=8$ does not
support. The binding limitation is therefore the observation space, exactly as
claimed, and not the amount of training: eight times the training budget leaves
the figure unchanged, whereas giving the agent the rule type moves it to full
compliance (Section~\ref{sec:conclusion}).
