...
(iv)~a \ac{LLM} cloud orchestrator (Gemini~3.1 Flash~Lite). On this workload the LLM attains
full semantic and anomaly compliance (69/69 and 8/8; Wilson 95\%~CI
94.7--100\% and 67.6--100\%) against 25\% anomaly compliance for the heuristic
baseline---under an exact paired McNemar test the only pairwise difference
reaching significance ($p=0.031$), the remaining orderings being
indistinguishable at $n=8$. Under an explicitly declared energy model (a model,
not a measurement) this costs three to four orders of magnitude more energy per
decision, framing the compliance-vs-efficiency trade-off for NTN-MEC
designers.
\end{abstract}
\begin{IEEEkeywords}
...
\begin{enumerate}
  ...
  \item A DRL agent trained with SB3 DQN over 50{,}000 synthetic steps using
    ...
    observing semantic rule types. We show analytically that the maximum for a
    binary-flag observation space is the share of anomalies that are
    mandatory discards---only those are identifiable from one bit---and verify
    over 20 seeds that the agent saturates it (29.0\% against a 28.3\%
    ceiling). The limit is therefore the observation space, not DRL as a
    paradigm nor the training budget.
  \item A characterisation of the compliance-vs-efficiency trade-off, reported
    with paired significance tests and interval estimates: the LLM resolved all
    semantic constraints presented to it, at an energy cost per decision that
    our declared model places three to four orders of magnitude above the
    heuristic baseline. We report which pairwise contrasts the sample size does
...
Because all four engines consume the identical task sequence the comparison is
\emph{paired}, admitting McNemar's exact test---more powerful than contrasting
independent rates. Even so, only one of the six pairwise contrasts is
significant at $\alpha=0.05$: LLM versus BASELINE (6 discordant pairs, all
favouring LLM; $p=0.031$). LLM versus DRL falls just short ($5/0$; $p=0.0625$),
SLM versus DRL is unresolved ($3/0$; $p=0.250$), and DRL versus BASELINE is
indistinguishable from noise ($2/3$; $p=1.000$). The ordering
BASELINE $<$ DRL $<$ SLM $<$ LLM is thus a consistent \emph{trend}, not an
established ranking: at $n=8$ a single reclassification moves ACR by 12.5~pp.
The contrast the abstract states---semantic-reasoning engine versus numerical
heuristic---is the one the evidence supports. Pooling 10--20 seeds
(80--160 anomaly-carrying tasks) would narrow the ACR intervals to
$\pm5$--$7$~pp and resolve the rest.

\paragraph*{Multi-seed replication}
We ran that replication for the two deterministic engines: 20 independent
seeds, 1{,}337 paired tasks, 138 anomaly-carrying. (The language-model engines
issue one API call per task and their replication is bounded by provider quota;
it is in progress.) Across seeds BASELINE reaches 26.8\% ACR (Wilson 95\%~CI
20.1--34.8) and DRL 29.0\% (22.1--37.0), with the paired difference
statistically null (McNemar $p=0.818$). The single-run DRL advantage over
BASELINE therefore \emph{does not replicate}, confirming the caution above.
What does replicate, and more sharply, is the information-theoretic argument.
The ceiling for any blind fallback equals the share of anomalies that are
mandatory-discard hardware failures---37.5\% (3 of 8) in the single run and
28.3\% (39 of 138) in the pooled sample---and DRL lands on it in both cases,
recovering 3 and 40 tasks respectively. The ceiling is thus a property of the
anomaly mix rather than a fixed constant, and the agent saturates it: the
binding limitation is the observation space, exactly as claimed, and not the
amount of training.
...
61.9\% (SAT-1, Brazil, 75~Wh, in sunlight), 21.8\% (SAT-2, Europe, 50~Wh, in
eclipse) and 53.4\% (SAT-15, USA, 100~Wh, in eclipse) \ac{SoC}. SAT-2 ended
...
This choice ($\approx$53\% eclipse against $\approx$35\% in the
literature~\cite{sumanth2019}) does not affect the engines symmetrically, and
the asymmetry runs \emph{against} the engines we report most favourably.
BASELINE is battery-greedy, so a deeper eclipse mostly reshuffles which
satellite it picks, leaving throughput intact; the rule-driven engines have no
such freedom, since a GDPR or sovereignty constraint pins the decision to one
region and a depleted satellite there leaves discarding as the only compliant
action. The pessimistic setting is therefore conservative with respect to our
conclusions. No battery-forced compliance drop materialised in this run---the
LLM's three drops are all mandatory hardware discards---so this is a structural
sensitivity argument; quantifying it needs an eclipse-fraction sweep.
...
of 69 calls rather than inference or propagation cost). From a throughput
...
\ac{LLM}---in a semantics-aware NTN-MEC simulation. On the workload studied,
the LLM resolved every semantic constraint it was given (69/69 tasks, 8/8
anomaly-carrying) by parsing natural-language rules, whereas BASELINE and DRL
reached 25\% and 38\% anomaly compliance
respectively, lacking any mechanism to read the rule. Under an exact paired
test, the LLM-versus-heuristic gap is statistically established ($p=0.031$);
the intermediate orderings are trends that the present sample size
($n=8$ anomaly-carrying tasks, single seed) cannot resolve.
...
latency and, under the declared per-decision energy model of
Section~\ref{sec:energy}, three to four orders of magnitude higher energy cost.
...
(3)~\emph{Reward-shaped learning vs.\ explicit rules}: reward shaping moved DRL
nominally above BASELINE on both metrics, although that specific gap is not
statistically resolvable here ($p=1.000$). The ceiling, however, is structural
and analytic rather than empirical---without observing the anomaly type, no
policy over this observation space can exceed 3/8, whereas zero-shot LLM/SLM
read the rule directly.

\subsection*{Limitations}
Three limitations bound these conclusions. First, the comparison is
\emph{asymmetric by construction}: DRL observes a single anomaly-present bit
while SLM and LLM receive the rule text. This isolates the value of semantic
parsing, but it means the 37.5\% ceiling characterises \emph{this observation
space}, not DRL as a paradigm---given a one-hot encoding of the three rule
types, a DRL agent (or a three-entry lookup table) should approach full
compliance. Second, the semantic space is small and closed: three enumerated
tokens whose ground truth is a three-branch predicate. That scale does not by
itself justify a language model, and these results should not be read as
establishing that it does; the claim a language model can uniquely support is
\emph{generalisation to constraints not enumerated at design time}, which this
experiment does not test. Third, the workload is light ($\lambda=4$~tasks/min,
12--14\% RAM occupancy), so the resource constraint never binds: the drop-rate
and utilisation findings describe an unsaturated regime, and the single-step
($\gamma=0$) formulation is appropriate precisely because temporal coupling is
weak. Our ongoing work addresses all three---an information-parity DRL arm and
a rule-table oracle, evaluated against a held-out corpus of unseen,
compositional natural-language constraints, under load high enough for RAM to
bind.

Further directions for future work include: (a)~a hybrid macro-micro architecture
